...
export function UTMAnalysis({ data }: UTMAnalysisProps) {
  ...
  const aggregateByField = (field: "campaign" | "medium" | "source") => {
    const aggregated = new Map<string, { visits: number; clicks: number; leads: number; revenue: number; conversions: number }>();

    filteredData.forEach(item => {
      const key = item[field];
      const existing = aggregated.get(key) || { visits: 0, clicks: 0, leads: 0, revenue: 0, conversions: 0 };
      aggregated.set(key, {
